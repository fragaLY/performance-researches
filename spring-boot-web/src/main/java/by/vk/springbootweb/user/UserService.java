...
public record UserService(UserRepository userRepository,
  ...
  public List<UserTransferResponse> userTransfers(Long userId) {
    ...
    return usersTransfersRepository.findByUserId(userId).parallelStream()
        .map(UserTransferResponse::from).collect(Collectors.toList());
  }

  public UserTransferResponse userTransfer(Long userId, Long transferId) {
    log.info("[USER SERVICE] Retrieving user's with id [{}] transfer with id [{}]", userId,
        transferId);
    return UserTransferResponse.from(
        usersTransfersRepository.findByUserIdAndTransferId(userId, transferId)
            .orElseThrow(() -> new NotFoundException("User's transfer not found")));
  }

  public void userTransfer(Long userId, Long transferId, UserTransferEditionPayload payload) {
    log.info("[USER SERVICE] Updating user's with id [{}] transfer with id [{}]", userId,
        transferId);
    var userTransfer = usersTransfersRepository.findByUserIdAndTransferId(userId, transferId)
        .orElseThrow(() -> new NotFoundException("User's transfer not found"));
  ...
  public void userTransfer(Long userId, Long transferId, UserTransferCreationPayload payload) {
    log.info("[USER SERVICE] Creating user's with id [{}] transfer with id [{}]", userId,
        transferId);
    var user = userRepository.findById(userId)
        .orElseThrow(() -> new NotFoundException("User not found"));
    var transfer = transferRepository.findById(transferId)
        .orElseThrow(() -> new NotFoundException("Transfer not found"));
    var userTransfer = new UsersTransfers(new UsersTransfersId(user.getId(), transfer.getId()),
        user, transfer, State.BOOKED, payload.description());
    usersTransfersRepository.save(userTransfer);
  }
}
