...
public class A2BSimulation extends Simulation {
  ...
  private static final Iterator<Map<String, Object>> FEEDER = Stream.generate(
      (Supplier<Map<String, Object>>) () -> Collections.singletonMap("userId",
          ThreadLocalRandom.current().nextLong(1, 200_001))).iterator();
  ...
  final ScenarioBuilder scenario = scenario("A2B simulation")
      ...
      .exec(http("[GET] The list of available countries is presented.")
          ...
          .check(jsonPath("$[:].id").findRandom().saveAs("countryId"))
      )
      .exitHereIfFailed()
      .pause(1, 3)
      .exec(http("[GET] The list of available cities is presented.")
          ...
          .check(status().is(HttpResponseStatus.OK.code()))
          .check(jsonPath("$[:].id").findRandom().saveAs("cityOriginId"))
      )
      .exitHereIfFailed()
      .pause(1, 3)
      .exec(http("[GET] The list of available origins is presented.")
          ...
          .check(status().is(HttpResponseStatus.OK.code()))
          .check(jsonPath("$[:].id").findRandom().saveAs("originId"))
      )
      .pause(3, 5)
      .exec(http("[GET] The list of available cities is presented.")
          .get("/countries/#{countryId}/cities")
          .check(status().is(HttpResponseStatus.OK.code()))
          .check(jsonPath("$[:].id").findRandom().saveAs("cityDestinationId"))
      )
      .exitHereIfFailed()
      .pause(1, 3)
      .exec(http("[GET] The list of available destinations is presented.")
          ...
          .check(status().is(HttpResponseStatus.OK.code()))
          .check(jsonPath("$[:].id").findRandom().saveAs("destinationId"))
      )
      .exitHereIfFailed()
      .pause(3, 5)
      .exec(http(
          "[GET] The list of available transfers by selected origin, destination, and date is presented.")
          .get("/transfers")
          ...
          .check(jsonPath("$[:].id").findRandom().saveAs("transferId"))
      )
      .exitHereIfFailed()
      .pause(3, 5)
      ...
      .exec(http("[PUT] Any (BOOKED) transfer description is updated.")
          ...
          .body(StringBody(
              "{\"state\": \"BOOKED\", \"description\": \"My new internal UUID " + UUID.randomUUID()
                  + "\" }"))
          .asJson()
      )
      .pause(1, 3)
      .exec(http("[PUT] Any (BOOKED) transfer is canceled (CANCELED).")
          ...
          .check(status().is(HttpResponseStatus.NO_CONTENT.code()))
          .body(StringBody(
              "{\"state\": \"CANCELED\", \"description\": \"My new internal UUID for canceled transfer "
                  + UUID.randomUUID() + "\" }"))
          .asJson()
      )
      .pause(1, 3)
      .exec(http("[PUT] Any (BOOKED) transfer is completed (COMPLETED).")
          ...
          .check(status().is(HttpResponseStatus.NO_CONTENT.code()))
          .body(StringBody(
              "{\"state\": \"COMPLETED\", \"description\": \"My new internal UUID for completed transfer "
                  + UUID.randomUUID() + "\" }"))
          .asJson()
      )
      .pause(1, 3)
      ...
      .exec(http("[PUT] User updates with her/his own data.")
          ...
          .check(status().is(HttpResponseStatus.NO_CONTENT.code()))
          .body(StringBody(
              "{ \"firstName\": \" NewFirstName#{userId} \", \"lastName\": \"NewLastName#{userId}\" }"))
          .asJson()
      )
      .pause(1, 5);
